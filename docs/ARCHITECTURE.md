...
- Генерация из одного из трёх **фиксированных пресетов** (`map-generator.service.ts`, выбор в админке с SVG-превью, `GET /sectors/admin/presets`):
  ...
  - **compact8** — тот же корпус, что у ring8, но без лепестков: ровный шестиугольник радиуса 4, 61 сектор (8 home_base, 16 easy, 24 medium, 6 hard, 6 особых, ядро). За базой ничего нет — все команды стартуют с края поля. Потолок — 8 команд.
- Во всех пресетах 6 особых (синих) секторов через один на кольце 2.
