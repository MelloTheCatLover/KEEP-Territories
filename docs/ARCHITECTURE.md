...
- Размер настраивается админом перед генерацией: 1..6 концентрических колец вокруг ядра. Default — 3 кольца (`hard`, `medium`, `easy` от центра наружу), 37 гексов.
- 6 home_base — в углах внешнего кольца, жёсткий потолок на 6 команд.
- Сложность задаётся **per-кольцо** в админ-конфиге. Нумерация внутри сложности — случайная (за исключением home base).
