...
// The map sits between two columns of team cards (3 per side for a 6-team game).
const CANVAS_W = 1600;
const CANVAS_H = 900;
const PAD = 20;
const GAP = 20;
...
export function AdminDisplayPage() {
  ...
  // One side of the board: cards stretch but stay capped, so 3 teams per column
  // read large without a single team's card ballooning to the full height.
  const column = (entries: SlotEntry[]) => (
    <div className="flex flex-col justify-center gap-3 min-h-0" style={{ width: COL_W }}>
      {entries.map((entry) => (
        <div key={entry.team.id} className="flex-1 min-h-0 max-h-[240px]">
          <ProjectorTeamCard
            team={entry.team}
            index={entry.index}
            pendingCount={pendingByTeam.get(entry.team.id) ?? 0}
            isLeadershipLeader={maxLeadership > 0 && entry.team.stats.leadership === maxLeadership}
          />
        </div>
      ))}
    </div>
